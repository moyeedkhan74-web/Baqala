...
import { HiUpload, HiDocumentText, HiPhotograph, HiCheckCircle, HiArrowRight, HiArrowLeft } from 'react-icons/hi';

const UploadApp = () => {
  const navigate = useNavigate();
  const [step, setStep] = useState(1);
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({ title: '', description: '', category: 'Games', version: '1.0.0', platform: 'Windows' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Create App Record
      const { data } = await api.post('/apps', formData);
      const appId = data.app._id;

      // 2. Upload Files
      const fileData = new FormData();
      fileData.append('appFile', files.appFile);
      fileData.append('icon', files.icon);
      files.screenshots.forEach(file => fileData.append('screenshots', file));

      const config = { 
        headers: { 'Content-Type': 'multipart/form-data' },
        onUploadProgress: (e) => {
          const percentCompleted = Math.round((e.loaded * 100) / e.total);
          toast.loading(`Deploying payload... ${percentCompleted}%`, { id: 'upload' });
        }
      };

      await api.post(`/apps/${appId}/upload`, fileData, config);
      toast.success('Project deployed successfully to the moderation queue!', { id: 'upload' });
      navigate('/developer');
    } catch (e) {
      toast.error('Deployment failed during transmission.', { id: 'upload' });
    } finally {
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 3 && (
              <div className="text-center py-10">
                ...
                <div className="glass-panel p-6 rounded-2xl text-left bg-dark-900/50 border-white/5 max-w-md mx-auto mb-8">
                  ...
                  <p className="text-sm text-gray-400 flex justify-between"><span>Files:</span> <span className="text-white font-medium">Binary + {1 + files.screenshots.length} Images</span></p>
                </div>
              </div>
            )}
